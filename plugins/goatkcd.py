...
def goatkcd(inp):
    ...
    if key == '': #if no comic number specific get random
	    key = 'random'
    ...
    if key != 'random': #check to make sure random comic actually exists
        try: http.open(goatkcd_url + strips + key + '.jpg').getcode() #check if given comic number actually exists
        except http.HTTPError: key = 'random' #if it doesn't then just use random
